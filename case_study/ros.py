...
class Callback(model.Task):
    def __init__(self, name, cb_type, scheduling_parameter, *args, **kwargs):
        ...
        self.scheduling_parameter = (cb_type, scheduling_parameter)

    def polling_based(self):
        return self.cb_type.polling_based()

    def bind_resource(self, r):
        super().bind_resource(r)
        if self.cb_type == CBType.EVENT_SOURCE:
    ...
    def __repr__(self):
        return "Callback({},{})".format(super().__repr__(), repr(self.cb_type))
...
def longest_unique_subchain(task, cross_resources=False):
    ...
    while len(start.prev_tasks) == 1 and (
        cross_resources or on_same_resource(start, start.prev_tasks[0])
    ):
...
class CBSReservation(model.Resource):
    def __init__(self, name=None, scheduler=None, budget=1, period=1, **kwargs):
        ...
        assert (
            int(budget) == budget and int(period) == period
        ), "Period and/or budget are not integer"
    ...
    def chains(self):
        ...
        ends = {
            ...
            if (
                (not t.next_tasks)
                or all([getattr(cb, "resource", None) != self for cb in t.next_tasks])
            )
        }
        while ends:
            e = ends.pop()
            ...
            ends.update(
                [
                    e
                    for e in newchain[0].prev_tasks
                    if getattr(e, "resource", None) == self
                ]
            )
            chains.add(newchain)
    ...
    def supply_bound(self, t):
        ...
        if not (tA < t <= tC):
            raise analysis.NotSchedulableException(
                "Numbers too large for python floats. System probably not schedulable"
            )
    ...
    def supply_time(self, supply):
        ...
        fractional_budget = (
            (slack + supply - full_budget) if full_budget < supply else 0
        )
...
def balance_rbf_sbf_updater(resource, compute_request):
    ...
    def updater(T):
        ...
        assert (
            resource.supply_bound(ret[0]) == requested
            and resource.supply_bound(ret[0] - 1) < requested
        )
        return ret
...
def latest_cb_response_at(task, A):
    ...
    if task.typ == CBType.TIMER:
        ...
        def compute_request(T):
            return task.request_bound(A + epsilon) + sum(
                [
                    t.request_bound(T - task.wcet + epsilon)
...
def latest_chain_response_at(chain, A):
    ...
    def compute_request(T):
        ...
        chn = chain_arr.eta_plus(T - sink.wcet + epsilon) * sum(
            cb.wcet for cb in chain_cbs[:-1]
        )
        oth = sum(
            longest_unique_subchain(cb, cross_resources=False)[
                0
            ].in_event_model.eta_plus(T - sink.wcet + epsilon)
            * cb.wcet
...
def max_busy_period_cb(task):
    ...
    if task.typ == CBType.EVENT_SOURCE:
        ...
        def compute_request(T):
            return (
                sum([t.request_bound(T) for t in hp_tasks])
                + max([t.wcet for t in lp_tasks])
...
def max_busy_period(chain):
    ...
    def compute_request(T):
        ret = sum(chain.request_bound(T) for chain in chain[0].resource.chains())
        return ret

    return fixed_point(
        balance_rbf_sbf_updater(chain[0].resource, compute_request), chain.wcet
    )
...
class DefaultScheduler:
    ...
    def compute_wcrt(self, task, task_results=None):
        ...
        if max_A > get_opt("max_wcrt"):
            raise analysis.NotSchedulableException(
                f"busy window {task} {max_A} > max wcrt {get_opt('max_wcrt')}"
            )
        for A in chain.request_bound_steps():
            ...
            if wcrt > get_opt("max_wcrt"):
                raise analysis.NotSchedulableException(
                    f"wcrt {task} {wcrt} > max wcrt {get_opt('max_wcrt')}"
                )
        if task_results:
